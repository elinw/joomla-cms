...
if (!empty($this->form->fieldsets)):
	...
	foreach ($this->form->fieldsets as $name => $fieldset) :
		if (!isset($fieldset->repeat) || isset($fieldset->repeat) && $fieldset->repeat == false)
		{

			$label = !empty($fieldset->label) ? JText::_($fieldset->label, true) : JText::_('COM_TAGS_'.$fieldset->name.'_FIELDSET_LABEL', true);
