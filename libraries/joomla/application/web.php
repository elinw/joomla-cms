 ...
 * @since       3.2
 */
abstract class JApplicationWeb extends JApplication
{
	/**
	...
	public function execute()
	{
		...
		if (!$this->response instanceof JApplicationWebResponse)
		{
	 ...
	 * @since  3.2
	 */
	public function setResponse(JApplicationWebResponse $response)
	{
	...
	protected function fetchClient()
	{
		...
		return new JApplicationWebClient;
	}
	...
	protected function loadSystemUris($requestUri = null)
	{
		...
		if ($siteUri != '')
		{
			$uri = new JUri($siteUri);
		}
		// No explicit base URI was set so we need to detect it.
		else
		{
			// Start with the requested URI.
			$uri = new JUri($this->provider->get('uri.request'));
